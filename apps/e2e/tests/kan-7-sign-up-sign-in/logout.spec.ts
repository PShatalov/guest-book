...
test.describe('Logout clears username and restores signed-out nav', () => {
  test('should restore guest nav after logout', async ({ page }) => {
    ...
    await expect(
      banner.getByText(username),
      'AC-6.3: username hidden',
    ).toHaveCount(0);
    await expect(
    ...
    await expect(banner.getByRole('button', { name: 'Log out' })).toHaveCount(
      0,
    );
  });
  ...
  test('should show signed-out nav after reload post-logout', async ({
    ...
    const sessionAfterLogout = page.waitForResponse(
      (r) => r.url().includes('/auth/session'),
      {
        timeout: TIMEOUT,
      },
    );
